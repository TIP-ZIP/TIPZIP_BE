...
public class MemberService {
    ...
    public MemberDTO getMemberFromNaver(String accessToken) {
        System.out.println("이제 네이버 서버에서 유저 정보를 가져오겠습니다.");
        //Webclient사용해 네이버 api 호출
        WebClient webClient = WebClient.create();
        String response = webClient.get()
        ...
        JsonParser parser = new JsonParser();
        JsonElement element = parser.parse(response);
        JsonObject naverAccount = element.getAsJsonObject();
        // JSON 응답 파싱
        JsonObject responseJson = JsonParser.parseString(response).getAsJsonObject();
        JsonObject responseBody = responseJson.getAsJsonObject("response");
        if (responseBody == null) {
            throw new RuntimeException("네이버 유저 정보가 없습니다.");
        }
        // 필드 값 확인 및 파싱
        String id = responseBody.has("id") ? responseBody.get("id").getAsString() : null;
        String nickname = responseBody.has("name") ? responseBody.get("name").getAsString() : null;
        String email = responseBody.has("email") ? responseBody.get("email").getAsString() : null;
        /*
        // 필요한 데이터 추출
        String nickname = naverAccount.get("name").getAsString();
        System.out.println("네이버에서 사용중인 닉네임: " + nickname);
        ...
        memberDTO.setSocial_id(nickname); // Social_id를 MemberDTO에 설정
        memberDTO.setEmail(email);
        memberDTO.setOAuthProvider(OAuthProvider.NAVER);

        boolean isNewMember=false;
        // 네이버로부터 받은 정보가 있을 경우 추가 설정
        if (memberDTO.getSocial_id() != null) {
            // 사용자 정보 확인을 위한 출력
            System.out.println("네이버에서 가져온 사용자 정보:");
            System.out.println("Member Name: " + memberDTO.getSocial_id());
            System.out.println("Member Email: " + memberDTO.getEmail()); // 예시로 이메일이 있다고 가정
